...
class Javday(Parser):
    ...
    def search(self, number):
        ...
        if self.specifiedUrl:
            ...
            self.detailurl = "https://javday.app/videos/" + self.number.replace("-", "") + "/"
    ...
    def getTitle(self, htmltree):
        ...
        # 删除番号和网站名
        result = title.replace(self.number, "").replace("- JAVDAY.APP", "").strip()
        return result

    def getTags(self, htmltree) -> list:
        tags = super().getTags(htmltree)
        return [tag for tag in tags if 'JAVDAY.APP' not in tag]
